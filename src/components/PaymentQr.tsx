...
function buildUpiUrl(upiId: string, payeeName: string) {
...
export function PaymentQr({ amount, compact = false }: { amount?: number; compact?: boolean }) {
  ...
  const upiId = `${shop?.phone}@ybl`;
  const payeeName = shop?.shopName || "Jewellery Shop";

  const upiUrl = buildUpiUrl(upiId, payeeName);
    ...
    <div className="w-full max-w-3xl rounded-md border border-slate-300 bg-white text-slate-900 overflow-hidden">
      ...
      <div className={`grid ${compact ? "grid-cols-[auto_1fr] gap-0" : "grid-cols-1 sm:grid-cols-[auto_1fr] gap-0"}`}>
        ...
        <div className={`flex flex-col items-center justify-center border-r border-slate-200 bg-slate-50/50 ${compact ? "p-2 w-28" : "p-3 w-36"} text-center`}>
          ...
          <div className={`break-all ${compact ? "text-[9px]" : "text-[10px]"} font-semibold leading-tight text-slate-900 mt-0.5`}>{upiId}</div>
        </div>
