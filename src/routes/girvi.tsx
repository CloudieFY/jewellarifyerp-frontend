...
import { inr, type Girvi, type GirviItem, type Customer, useLocalState } from "@/lib/storage";
...
function calculateInterest(girvi: any) {
  ...
  return Math.round(calculateGirviInterest(P, monthlyRatePct, totalMonths).interest);
}
...
export default function GirviPage() {
  ...
  const { data: girvis = [], isLoading } = useQuery<Girvi[]>({ queryKey: ["girvis"], queryFn: api.girvi.getAll });
  ...
  const createMutation = useApiMutation((data: Girvi) => api.girvi.create(data), ["girvis"]);
  const updateMutation = useApiMutation((data: { id: string; body: Girvi }) => api.girvi.update(data.id, data.body), ["girvis"]);
  const deleteMutation = useApiMutation((id: string) => api.girvi.remove(id), ["girvis"]);
  ...
  async function add(createInvoice = false) {
    if (!form.loanAmount) {
      toast.error(t("girvi.toast.loanAmountRequired"));
      return;
    }
    if (form.customerMobile !== "NEW" && !form.customerName) {
      toast.error(t("girvi.toast.customerSelectionRequired"));
      return;
    }
    if (!form.items || form.items.length === 0) {
      toast.error(t("girvi.toast.atLeastOneItem"));
      return;
    }
    for (const item of form.items) {
      if (!item.itemDescription) {
        toast.error(t("girvi.toast.itemDescriptionRequired"));
        return;
      }
    }
    ...
    try {
      ...
      if (editingId) {
        ...
        toast.success(t("girvi.toast.updatedSuccess"));
      } else {
        saved = await createMutation.mutateAsync(payload);
        toast.success(t("girvi.toast.savedSuccess"));
      }
      setForm({
        ...form,
        loanNo: `GL-${Date.now().toString().slice(-6)}`,
      ...
      toast.error(t("girvi.toast.failedConnect"));
    }
  }

  const handleKeyNav = useFormKeyboardNav(() => add(false));
  ...
  async function setStatus(id: string, status: Girvi["status"]) {
    ...
    if ((status === "Closed" || status === "Auctioned") && g) {
      if (Number(g.forwardedAmount) > 0 && !isGirviForwardedSettled(g)) {
        window.alert(t("girvi.toast.takeFromForwardedFirst"));
        return;
      }
    }
    ...
    if (status === "Closed" && g) {
      ...
      const confirmed = window.confirm(t("girvi.toast.confirmClose", { principal: inr(g.loanAmount), interest: inr(interest), total: inr(total) }));
      if (!confirmed) return;
    }
    if (g) await updateMutation.mutateAsync({ id, body: { ...g, status } });
  }
  async function remove(id: string) {
    if (!window.confirm(t("girvi.toast.confirmDelete"))) return;
  ...
  return (
    <Layout>
      ...
      <header className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-6">
        <div>
          <h1 className="text-4xl">{t("girvi.title")}</h1>
          ...
          <DialogContent className="fixed inset-0 z-[100] w-screen h-screen max-w-none max-h-none translate-x-0 translate-y-0 top-0 left-0 rounded-none border-0 p-3 sm:p-5 bg-neutral-50 dark:bg-slate-950 flex flex-col overflow-y-auto shadow-none" aria-describedby={undefined} onInteractOutside={(e) => e.preventDefault()} onKeyDown={handleKeyNav}>
            ...
            <div className="p-4 sm:p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <Label className="text-xs">{t("girvi.searchCustomer")}</Label>
                  <Input
                ...
                <div className="space-y-4" ref={itemsContainerRef}>
                  {(form.items || []).map((item, idx) => (
                    <div key={idx} className="p-3 rounded-md border border-input bg-muted/30 space-y-3 relative">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <Label className="text-xs">{t("girvi.type")}</Label>
                          <Select value={item.itemType} onValueChange={(v) => updateItem(idx, { itemType: v as any })}>
                            <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                            <SelectContent>
                              <SelectItem value="Gold">{translateEnum(metalTypeMap, "Gold", language)}</SelectItem>
                              <SelectItem value="Silver">{translateEnum(metalTypeMap, "Silver", language)}</SelectItem>
                              <SelectItem value="Mixed">{translateEnum(metalTypeMap, "Mixed", language)}</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                        <div>
                          <Label className="text-xs">{t("girvi.category")}</Label>
                          <Select value={item.itemCategory || ""} onValueChange={(v) => updateItem(idx, { itemCategory: v })}>
                            <SelectTrigger className="h-8 text-xs"><SelectValue placeholder={t("girvi.selectPlaceholder")} /></SelectTrigger>
                            <SelectContent>
                              {categories.map((cat) => (
                                <SelectItem key={cat} value={cat}>{translateEnum(ornamentTypeMap, cat, language)}</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                        </div>
                      ...
                      <div>
                        <Label className="text-xs">{t("girvi.descriptionRequired")}</Label>
                        <Input data-desc-idx={idx} className="h-8 text-xs" value={item.itemDescription} onChange={(e) => updateItem(idx, { itemDescription: e.target.value })} placeholder={t("girvi.descriptionPlaceholder")} />
                      </div>

                      <div className="grid grid-cols-3 gap-2">
                        <div>
                          <Label className="text-xs">{t("girvi.grossG")}</Label>
                          <Input type="number" className="h-8 text-xs" value={item.grossWeight || ""} onChange={(e) => updateItem(idx, { grossWeight: +e.target.value })} />
                        </div>
                        <div>
                          <Label className="text-xs">{t("girvi.netG")}</Label>
                          <Input type="number" className="h-8 text-xs" value={item.netWeight || ""} onChange={(e) => updateItem(idx, { netWeight: +e.target.value })} />
                        </div>
                        <div>
                          <Label className="text-xs">{t("girvi.purity")}</Label>
                          <Input
                            ...
                            onKeyDown={
                              idx === (form.items || []).length - 1
                                ? (e) => {
                                    ...
                                    e.preventDefault();
                                    e.stopPropagation(); // last row — add a new one and jump straight into it
                                    addItem();
                                    requestAnimationFrame(() => {
                                      itemsContainerRef.current
                                        ?.querySelector<HTMLInputElement>(`[data-desc-idx="${idx + 1}"]`)
                                        ?.focus();
                                    });
                                  }
                                : undefined
                ...
                {(form.items || []).length > 0 && (
                  <div className="mt-3 p-2 rounded-md bg-muted/50 text-xs">
                    <div className="grid grid-cols-2 gap-2 font-medium">
                      <div>{t("girvi.totalGross")}: {getTotalWeights().totalGross.toFixed(3)} g</div>
                      <div>{t("girvi.totalNet")}: {getTotalWeights().totalNet.toFixed(3)} g</div>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div>
                <Label>{t("girvi.note")}</Label>
                <Textarea rows={2} value={form.note || ""} onChange={(e) => setForm({ ...form, note: e.target.value })} />
              </div>

              <div className="border-t pt-4 mt-2">
                <h3 className="text-sm font-medium mb-3 text-purple-700">{t("girvi.forwardingDetailsOptional")}</h3>
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <div>
                    <Label>{t("girvi.shopName")}</Label>
                    <div className="flex gap-1">
                      <Input value={form.forwardedShopName || ""} onChange={(e) => setForm({ ...form, forwardedShopName: e.target.value })} placeholder={t("girvi.shopName")} className="flex-1" />
                  ...
                  <div>
                    <Label>{t("girvi.shopGstNoOptional")}</Label>
                    <Input value={form.forwardedShopGstNo || ""} onChange={(e) => setForm({ ...form, forwardedShopGstNo: e.target.value })} placeholder="GSTIN" />
                  </div>
                </div>
                <div className="mb-2">
                  <Label>{t("girvi.shopAddress")}</Label>
                  <Textarea rows={2} value={form.forwardedShopAddress || ""} onChange={(e) => setForm({ ...form, forwardedShopAddress: e.target.value })} placeholder={t("girvi.shopAddress")} />
                </div>
                <div className="grid grid-cols-3 gap-2 mb-2">
                  <div>
                    <Label>{t("girvi.forwardedDate")}</Label>
                    <Input type="date" value={form.forwardedDate || ""} onChange={(e) => setForm({ ...form, forwardedDate: e.target.value })} />
                  </div>
                  <div>
                    <Label>{t("girvi.forwardedAmountRs")}</Label>
                    <Input type="number" value={form.forwardedAmount || ""} onChange={(e) => setForm({ ...form, forwardedAmount: +e.target.value })} />
                  </div>
                  <div>
                    <Label>{t("girvi.shopInterestRatePct")}</Label>
                ...
                <div>
                  <Label>{t("girvi.forwardedItemImage")}</Label>
              ...
              <div className="border-t pt-4 mt-2 mb-2">
                <Label className="text-muted-foreground font-medium block mb-3">{t("girvi.signaturesOptional")}</Label>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <Label className="text-xs">{t("girvi.customerSignature")}</Label>
                  ...
                  <div>
                    <Label className="text-xs">{t("girvi.authorizedSignatory")}</Label>
                    <Input type="file" accept="image/*" className="bg-background mt-1" onChange={(e) => {
                      const file = e.target.files?.[0];
                      if (file) {
                        const reader = new FileReader();
              ...
              <div className="grid grid-cols-1 gap-2 md:grid-cols-2 pt-2">
                <Button className="w-full" onClick={() => add(false)}>{t("girvi.saveGirvi")}</Button>
                <Button className="w-full" onClick={() => add(true)}>{t("girvi.saveAndPrint")}</Button>
              </div>
            </div>
          </DialogContent>
        </Dialog>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <KPI label={t("girvi.kpi.activeLoans")} value={totals.activeCount} icon={Landmark} colorClass="text-primary" />
        <KPI label={t("girvi.kpi.principalOut")} value={formatCompactIfLarge(totals.principal)} icon={Wallet} colorClass="text-amber-600" />
        <KPI label={t("girvi.kpi.pledgedWeight")} value={`${totals.pledgedWeight.toFixed(3)} g`} icon={Scale} colorClass="text-blue-600" />
        <KPI label={t("girvi.kpi.collateralValue")} value={formatCompactIfLarge(totals.collateralValue)} icon={TrendingUp} colorClass="text-emerald-600" />
      </div>

      {totals.forwardedPrincipal > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
          <KPI label={t("girvi.kpi.forwardedPrincipal")} value={formatCompactIfLarge(totals.forwardedPrincipal)} icon={Landmark} colorClass="text-purple-600" />
          <KPI label={t("girvi.kpi.shopInterestDue")} value={formatCompactIfLarge(totals.forwardedInterest)} icon={TrendingUp} colorClass="text-rose-500" />
          <KPI label={t("girvi.kpi.totalShopPayable")} value={formatCompactIfLarge(totals.forwardedPrincipal + totals.forwardedInterest)} icon={Wallet} colorClass="text-rose-700" />
        </div>
      )}
      ...
      <Card className="shadow-sm border-border overflow-hidden flex flex-col">
          <CardHeader className="bg-muted/20 border-b border-border pb-3 pt-4">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <CardTitle className="text-base font-semibold font-display">{t("girvi.loanRecords")}</CardTitle>
              <div className="flex items-center gap-2 w-full sm:w-auto">
                ...
                <Select value={filter} onValueChange={(v) => setFilter(v as typeof filter)}>
                  <SelectTrigger className="w-32 h-8 bg-background text-xs font-medium border-border shadow-sm"><SelectValue placeholder={t("common.status")} /></SelectTrigger>
                  <SelectContent>
                    <SelectItem value="All">{t("girvi.allStatus")}</SelectItem>
                    <SelectItem value="Active">{translateEnum(girviStatusMap, "Active", language)}</SelectItem>
                    <SelectItem value="Closed">{translateEnum(girviStatusMap, "Closed", language)}</SelectItem>
                    <SelectItem value="Auctioned">{translateEnum(girviStatusMap, "Auctioned", language)}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
          ...
          <CardContent className="p-0">
            {isLoading ? <p className="text-sm text-muted-foreground py-12 text-center">{t("girvi.loadingRecords")}</p> : filtered.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
                <Search className="w-10 h-10 mb-3 opacity-20" />
                <p>{t("girvi.noRecords")}</p>
              </div>
            ) : (
            <div>
              {/* Desktop Table View */}
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-sm text-left border-collapse min-w-[850px]">
                  <thead className="bg-muted/40 text-muted-foreground text-[11px] uppercase tracking-wider border-b border-border">
                    <tr>
                      <th className="py-3 px-3 sm:px-4 font-semibold">{t("girvi.table.loanNo")}</th>
                      <th className="py-3 px-3 sm:px-4 font-semibold">{t("girvi.table.dateTime")}</th>
                      <th className="py-3 px-3 sm:px-4 font-semibold">{t("girvi.table.customer")}</th>
                      <th className="py-3 px-3 sm:px-4 font-semibold">{t("girvi.table.itemDetails")}</th>
                      <th className="py-3 px-3 sm:px-4 font-semibold text-right">{t("girvi.table.netWt")}</th>
                      <th className="py-3 px-3 sm:px-4 font-semibold text-right">{t("girvi.table.principal")}</th>
                      <th className="py-3 px-3 sm:px-4 font-semibold text-right">{t("girvi.table.interest")}</th>
                      <th className="py-3 px-3 sm:px-4 font-semibold text-center">{t("girvi.table.status")}</th>
                      <th className="py-3 px-3 sm:px-4 font-semibold text-right pr-6">{t("girvi.table.action")}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginated.map((g) => {
                      ...
                      return (
                        <tr key={(g as any)._id || g.id} className="border-b border-border/50 last:border-0 align-top hover:bg-muted/20 transition-colors">
                          ...
                          <td className="py-3 px-3 sm:px-4 min-w-25">
                            ...
                            <div className="text-[11px] text-muted-foreground font-medium mt-0.5">{getElapsedTimeString(g.date)} {t("girvi.elapsed")}</div>
                            {g.dueDate && <div className="text-[11px] text-rose-500 font-medium mt-0.5">{t("girvi.due")}: {formatDate(g.dueDate)}</div>}
                          </td>
                          ...
                          <td className="py-3 px-3 sm:px-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                {hasMultipleItems ? (
                                  <div className="font-medium text-foreground">{g.items?.length || 0} {t("girvi.items")}</div>
                                ) : (
                                  <div className="font-medium text-foreground">{translateEnum(metalTypeMap, g.itemType, language)} {g.purity}</div>
                                )}
                              ...
                              {(g.forwardedShopName || g.forwardedTo) && (
                                <div className={`mt-1 text-[10px] font-semibold border inline-block px-1.5 py-0.5 rounded truncate max-w-37.5 lg:max-w-xs ${(g as any).isForwardedSettled ? "text-green-700 border-green-200 bg-green-50" : "text-purple-700 border-purple-200 bg-purple-50"}`} title={g.forwardedShopName || g.forwardedTo}>
                                  {t("girvi.fwd")}: {g.forwardedShopName || g.forwardedTo} {(g as any).isForwardedSettled ? t("girvi.settledSuffix") : ""}
                                </div>
                              )}
                              </div>
                            </div>
                          ...
                          <td className="py-3 px-3 sm:px-4 text-right min-w-27.5">
                            ...
                            <div className="text-[10px] leading-tight text-muted-foreground font-medium mt-0.5">@ {g.interestPct}{t("girvi.perMonth")}<br/>{t("girvi.compoundMonthly")}</div>
                          </td>
                          ...
                          <td className="py-3 px-3 sm:px-4 text-right">
                            <div className="flex justify-end gap-1">
                              <Button size="sm" variant="outline" className="h-8 bg-background" onClick={() => setViewing(g as Girvi)}>{t("girvi.view")}</Button>
              ...
              <div className="md:hidden grid grid-cols-1 sm:grid-cols-2 gap-3 p-3">
                {paginated.map((g) => {
                  ...
                  return (
                    <div key={(g as any)._id || g.id} className="p-3.5 rounded-xl border border-border bg-card shadow-sm space-y-3">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <div className="font-mono text-xs font-bold text-primary">{g.loanNo}</div>
                          <div className="font-semibold text-base text-foreground mt-0.5">{g.customerName}</div>
                          <div className="text-xs text-muted-foreground">{g.customerMobile}</div>
                        </div>
                        <Select value={g.status} onValueChange={(v) => setStatus((g as any)._id || g.id, v as Girvi["status"])}>
                          <SelectTrigger className={`h-7 w-24 text-[10px] font-bold uppercase tracking-wider border ${statusColors[g.status] || ""}`}>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="Active">{translateEnum(girviStatusMap, "Active", language)}</SelectItem>
                      ...
                      <div className="flex items-center gap-3 p-2 rounded-lg bg-muted/40 text-xs">
                        <div className="w-10 h-10 rounded bg-background border border-border overflow-hidden shrink-0 flex items-center justify-center">
                          {g.imageUrl ? <img src={g.imageUrl} alt="Item" className="w-full h-full object-cover" /> : <ImageIcon className="w-4 h-4 text-muted-foreground" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-foreground truncate">
                            {hasMultipleItems ? `${g.items?.length || 0} ${t("girvi.items")}` : `${translateEnum(metalTypeMap, g.itemType, language)} ${g.purity}`}
                          </div>
                          <div className="text-[11px] text-muted-foreground truncate">
                            {hasMultipleItems ? (g.items || []).map(it => it.itemDescription).join(', ') : g.itemDescription}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 py-2 px-2.5 rounded-lg bg-muted/30 text-center text-xs">
                        <div>
                          <div className="text-[10px] text-muted-foreground uppercase">Net Wt</div>
                          <div className="font-semibold mt-0.5">{(totalNetWeight || 0).toFixed(3)}g</div>
                        </div>
                        <div>
                          <div className="text-[10px] text-muted-foreground uppercase">Loan Amount</div>
                          <div className="font-semibold text-foreground mt-0.5">{inr(g.loanAmount)}</div>
                        </div>
                        <div>
                          <div className="text-[10px] text-muted-foreground uppercase">Interest</div>
                          <div className="font-semibold text-amber-600 mt-0.5">{inr(interestAmt)}</div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-1 border-t border-border/60 text-xs">
                        ...
                        <div className="flex items-center gap-1">
                          <Button size="sm" variant="outline" className="h-8 text-xs gap-1" onClick={() => setViewing(g as Girvi)}>
                            {t("girvi.view")}
                          </Button>
                          <Button size="sm" variant="outline" className="h-8 text-xs gap-1" onClick={() => startEdit(g as Girvi)}>
                            <Pencil className="w-3.5 h-3.5 text-muted-foreground" /> Edit
                          </Button>
                          <Button size="sm" variant="ghost" className="h-8 w-8 p-0 text-rose-500" onClick={() => remove((g as any)._id || g.id)}>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 border-t">
                  <div className="text-xs text-muted-foreground">{t("girvi.showingEntries", { from: (currentPage - 1) * 10 + 1, to: Math.min(currentPage * 10, filtered.length), total: filtered.length })}</div>
                  <div className="flex gap-1">
                    <Button size="sm" variant="outline" onClick={() => setPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}>{t("girvi.prev")}</Button>
                    <Button size="sm" variant="outline" onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages}>{t("girvi.next")}</Button>
                  </div>
                </div>
              )}
            </div>
  ...
  return (
    <div className="print-section fixed inset-0 z-100 bg-black/50 flex justify-center items-start p-2 sm:p-4 print:static print:block print:bg-white print:p-0 print:overflow-visible print:h-auto overflow-y-auto pointer-events-auto">
      <div className="bg-white w-full max-w-3xl rounded-lg shadow-xl print:shadow-none print:max-w-none text-slate-900 my-auto relative flex flex-col max-h-[95vh] print:my-0 print:max-h-none print:block">
        <style>{`@media print { @page { size: A4; margin: 4mm; } body { zoom: 0.9; } }`}</style>
        <div className="p-4 sm:p-6 print:p-2 border-2 border-transparent print:border-none m-2 print:m-0 bg-white overflow-y-auto flex-1 print:overflow-visible">
          <ShopHeader documentLabel={t("girvi.print.documentLabel")} compact />
          ...
          <div className="flex justify-between items-start mb-3 text-sm">
            <div>
              ...
              <div className="font-bold text-base leading-tight">{girvi.customerName}</div>
              <div className="text-slate-700 text-xs">{girvi.customerMobile} {girvi.customerMobile2 ? `/ ${girvi.customerMobile2}` : ""}</div>
              <div className="text-slate-700 text-xs mt-1">{girvi.customerAddress || t("girvi.print.noAddress")}</div>
            </div>
            <div className="text-right">
              <table className="ml-auto text-left text-slate-700 text-xs">
                <tbody>
                  <tr><td className="pr-3 py-0.5 text-right font-medium text-slate-500">{t("girvi.table.loanNo")}:</td><td className="font-semibold text-slate-900">{girvi.loanNo}</td></tr>
                  <tr><td className="pr-3 py-0.5 text-right font-medium text-slate-500">{t("girvi.print.status")}:</td><td className="font-semibold text-slate-900 uppercase">{translateEnum(girviStatusMap, girvi.status, language)}</td></tr>
                  <tr><td className="pr-3 py-0.5 text-right font-medium text-slate-500">{t("girvi.print.date")}:</td><td className="font-semibold text-slate-900">{formatDate(girvi.date)}</td></tr>
                  {girvi.dueDate && (
                    <tr><td className="pr-3 py-0.5 text-right font-medium text-slate-500">{t("girvi.print.dueDate")}:</td><td className="font-semibold text-slate-900">{formatDate(girvi.dueDate)}</td></tr>
                  )}
                  {girvi.documentNumber && (
                    <tr><td className="pr-3 py-0.5 text-right font-medium text-slate-500">{t("girvi.billInvoiceNo")}:</td><td className="font-semibold text-slate-900">{girvi.documentNumber}</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          {/* Item Details Table */}
          <div className="mb-3">
            <h3 className="font-bold text-[10px] uppercase text-slate-500 mb-1 tracking-wider">{t("girvi.print.pledgedItemDetails")}</h3>
            <div className="overflow-x-auto w-full">
              <table className="w-full text-xs border-collapse border border-slate-300 min-w-150 print:min-w-full">
                <thead className="bg-slate-100">
                <tr>
                  <th className="border border-slate-300 p-1.5 text-left font-semibold">{t("girvi.print.description")}</th>
                  <th className="border border-slate-300 p-1.5 text-left font-semibold">{t("girvi.print.type")}</th>
                  <th className="border border-slate-300 p-1.5 text-right font-semibold">{t("girvi.print.grossWt")}</th>
                  <th className="border border-slate-300 p-1.5 text-right font-semibold">{t("girvi.print.netWt")}</th>
                  <th className="border border-slate-300 p-1.5 text-right font-semibold">{t("girvi.print.purity")}</th>
                </tr>
              </thead>
              <tbody>
                {(girvi.items && girvi.items.length > 0 ? girvi.items : [{
                  ...
                  <tr key={idx}>
                    ...
                    <td className="border border-slate-300 p-1.5">{translateEnum(metalTypeMap, item.itemType, language)} {item.itemCategory ? `- ${translateEnum(ornamentTypeMap, item.itemCategory, language)}` : ""}</td>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start text-xs gap-4">
            <div className="w-full sm:w-1/2 sm:pr-8 order-2 sm:order-1">
              {girvi.status === "Closed" && (
                <div className="p-1.5 bg-green-50 border border-green-200 text-green-800 text-center font-bold rounded tracking-widest text-base">
                  {t("girvi.print.loanClosedCleared")}
                </div>
              )}
              <div className="mt-4 text-slate-600">
                <div className="flex justify-between mb-1"><span>{t("girvi.print.interestRate")}:</span><span className="font-bold">{girvi.interestPct}{t("girvi.print.perMonthCompound")}</span></div>
                <div className="flex justify-between mb-1"><span>{t("girvi.print.timeElapsed")}:</span><span className="font-bold">{getElapsedTimeString(girvi.date)}</span></div>
                <div className="flex justify-between"><span>{t("girvi.print.estMarketValue")}:</span><span className="font-bold">{inr(girvi.marketValue || 0)}</span></div>
              </div>
            </div>
            <div className="w-full sm:w-1/2 max-w-sm order-1 sm:order-2">
              <table className="w-full">
                <tbody>
                  <tr>
                    <td className="py-0.5 text-slate-600">{t("girvi.print.principalAmount")}</td>
                    <td className="py-0.5 text-right font-semibold">{inr(girvi.loanAmount)}</td>
                  </tr>
                  {girvi.status !== "Closed" && (
                    <tr>
                      <td className="py-0.5 text-slate-600">{t("girvi.print.accruedInterest")}</td>
                      <td className="py-0.5 text-right font-semibold">{inr(interest)}</td>
                    </tr>
                  )}
                  <tr className="border-t-2 border-slate-300 text-sm">
                    <td className="py-1 font-bold text-slate-900">{girvi.status === "Closed" ? t("girvi.print.balanceDue") : t("girvi.print.totalPayable")}</td>
                    <td className={`py-1 text-right font-bold ${girvi.status === "Closed" ? "text-green-700" : "text-rose-700"}`}>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4 mt-4">
            <div>
              {displayNote && (
                <div className="text-xs">
                  <strong className="text-xs uppercase text-slate-500 mb-1 block tracking-wider">{t("girvi.print.remarksNote")}</strong>
            ...
            <div className="text-right flex justify-end">
              {girvi.imageUrl && (
                <div className="text-xs">
                   <strong className="text-xs uppercase text-slate-500 mb-1 block tracking-wider">{t("girvi.print.pledgedItem")}</strong>
          ...
          {(girvi.forwardedShopName || girvi.forwardedTo) && (
            <div className="mb-4 p-3 border border-purple-200 bg-purple-50 rounded text-xs print:hidden">
              <h4 className="font-bold mb-3 uppercase tracking-wider text-purple-900 flex items-center gap-2">
                <span className="bg-purple-200 text-purple-900 px-2 py-0.5 rounded">{t("girvi.print.internalUseOnly")}</span>
                {t("girvi.print.forwardingDetails")}
                {isGirviForwardedSettled(girvi) && <span className="ml-auto bg-green-100 text-green-800 px-2 py-0.5 rounded text-[10px]">{t("girvi.print.settled")}</span>}
              </h4>
              <div className="grid grid-cols-3 gap-4 text-purple-900">
                <div>
                  <div className="text-purple-600/80 mb-0.5">{t("girvi.print.shopLocation")}</div>
                ...
                <div>
                  <div className="text-purple-600/80 mb-0.5">{t("girvi.print.shopFinancials")}</div>
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1">
                    <span>{t("girvi.print.forwardedAmount")}</span> <span className="font-bold">{inr(girvi.forwardedAmount || 0)}</span>
                    <span>{t("girvi.print.fwdDate")}</span> <span className="font-bold">{girvi.forwardedDate ? formatDate(girvi.forwardedDate) : formatDate(girvi.date)}</span>
                    <span>{t("girvi.print.rate")}</span> <span className="font-bold">{girvi.forwardedInterestPct || 0}{t("girvi.print.perMonthCompound")}</span>
                    <span>{isGirviForwardedSettled(girvi) ? t("girvi.print.settledInterest") : t("girvi.print.interestLabel")}</span> <span className="font-bold text-amber-700">{inr(forwardedInterest)}</span>
                    {isGirviForwardedSettled(girvi) && ((girvi as any).forwardedSettledDate || girvi.note?.match(/cleared on (.*?) - Paid/)) && (
                      <>
                        <span>{t("girvi.print.settledOn")}</span> <span className="font-bold">{girvi.note?.match(/cleared on (.*?) - Paid/)?.[1] || formatDate((girvi as any).forwardedSettledDate)}</span>
                      </>
                    )}
                    <span className="font-bold pt-1 border-t border-purple-200">{isGirviForwardedSettled(girvi) ? t("girvi.print.totalSettledAmount") : t("girvi.print.totalOwed")}</span> <span className="font-bold text-rose-700 pt-1 border-t border-purple-200">{inr(forwardedTotal)}</span>
                  </div>
                </div>
          ...
          <div className="mt-12 print:mt-6 grid grid-cols-1 sm:grid-cols-2 gap-8 items-end text-[10px] font-bold text-slate-500 uppercase tracking-wider print:break-inside-avoid">
            <div className="text-center">
              {girvi.customerSignature && girvi.customerSignature !== "data:," ? (
              ...
              {t("girvi.print.customerSignature")}
            </div>
            <div className="text-center">
              {girvi.authorizedSignatory && girvi.authorizedSignatory !== "data:," ? (
                <img src={girvi.authorizedSignatory} alt="Authorized Signatory" className="h-16 mx-auto mb-2 object-contain" />
              ) : (
                <div className="w-32 border-t border-slate-300 mb-1 mx-auto"></div>
              )}
              {t("girvi.print.authorizedSignatory")}
            </div>
          </div>
          ...
          <div className="mt-8 print:mt-2 border-t border-slate-200 pt-4 print:pt-2 text-center text-xs normal-case tracking-normal font-normal text-slate-600 print:break-inside-avoid">
            {authUser?.termsAndConditions ? (
              <div className="text-xs text-slate-600 whitespace-pre-wrap">{authUser.termsAndConditions}</div>
            ) : (
        ...
        <div className="shrink-0 bg-slate-100 p-4 border-t border-slate-200 rounded-b-lg flex justify-end gap-3 print:hidden">
          <Button variant="outline" onClick={onClose}>{t("girvi.print.close")}</Button>
          <Button onClick={triggerPrint}>
            <Printer className="w-4 h-4 mr-2" /> {t("girvi.print.printReceipt")}
          </Button>
        </div>
      </div>
    </div>
