...
export default function AdvancePage() {
  ...
  const { data: advances = [], isLoading } = useQuery({ queryKey: ["advances"], queryFn: api.advances.getAll });
  const { data: customers = [] } = useQuery<Customer[]>({ queryKey: ["customers"], queryFn: api.customers.getAll });
  const createMutation = useApiMutation((data: Advance) => api.advances.create(data), ["advances"]);
  const updateMutation = useApiMutation((data: { id: string; body: Advance }) => api.advances.update(data.id, data.body), ["advances"]);
  const deleteMutation = useApiMutation((id: string) => api.advances.remove(id), ["advances"]);
  ...
  async function setStatus(id: string, status: Advance["status"]) {
    const adv = advances.find((a) => (a.id || (a as any)._id) === id);
    if (adv && id) await updateMutation.mutateAsync({ id, body: { ...adv, status } });
  }
